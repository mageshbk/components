...
public final class EndpointPublisherImpl implements EndpointPublisher {
    ...
    public Context publish(String context, ClassLoader loader, Map<String, String> urlPatternToClassNameMap, WebservicesMetaData metadata) throws Exception {
        return publish(null, context, loader, urlPatternToClassNameMap, metadata, null);
    }
    ...
    public Context publish(String context, ClassLoader loader, Map<String, String> urlPatternToClassNameMap, WebservicesMetaData metadata, JBossWebservicesMetaData jbwsMetadata) throws Exception {
        return publish(null, context, loader, urlPatternToClassNameMap, metadata, jbwsMetadata);
    }

    public Context publish(ServiceTarget target, String context, ClassLoader loader, Map<String, String> urlPatternToClassNameMap, WebservicesMetaData metadata, JBossWebservicesMetaData jbwsMetadata) throws Exception {
        WSEndpointDeploymentUnit unit = new WSEndpointDeploymentUnit(loader, context, urlPatternToClassNameMap, metadata);
        return new Context(context, publish(target, unit, jbwsMetadata));
    }

    public List<Endpoint> publish(ServiceTarget target, WSEndpointDeploymentUnit unit) throws Exception {
        return publish(target, unit, null);
    }

    /**
     * Publishes the endpoints declared to the provided WSEndpointDeploymentUnit
     */
    public List<Endpoint> publish(ServiceTarget target, WSEndpointDeploymentUnit unit, JBossWebservicesMetaData jbwsMetadata) throws Exception {
        List<DeploymentAspect> aspects = DeploymentAspectsProvider.getSortedDeploymentAspects();

        if (jbwsMetadata != null) {
        System.out.println("Setting config file ... " + jbwsMetadata.getConfigFile());
            BusDeploymentAspect bda = new BusDeploymentAspect(jbwsMetadata.getConfigName(), jbwsMetadata.getConfigFile());
            for (DeploymentAspect aspect : aspects) {
                if (aspect instanceof org.jboss.wsf.stack.cxf.deployment.aspect.BusDeploymentAspect) {
                    aspects.set(aspects.indexOf(aspect), bda);
                    break;
                }
            }
        }
        ...
        System.out.println("done ............................................ ");
            ...
            if (target == null) {
                ...
                String configName = EndpointConfig.STANDARD_ENDPOINT_CONFIG;
                String configFile = null;
                if (jbwsMetadata != null) {
                    if (jbwsMetadata.getConfigName() != null) {
                        configName = jbwsMetadata.getConfigName();
                    }
                    configFile = jbwsMetadata.getConfigFile();
                }
